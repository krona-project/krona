    ...
    public abstract class Transaction : IEquatable<Transaction>, IInventory
    {
        ...
        public virtual Fixed8 NetworkFee
        {
            get
            {
                ...
                {
                    Fixed8 input = References.Values.Where(p => p.AssetId.Equals(Blockchain.GoverningToken.Hash)).Sum(p => p.Value);
                    Fixed8 output = Outputs.Where(p => p.AssetId.Equals(Blockchain.GoverningToken.Hash)).Sum(p => p.Value);
                    _network_fee = input - output;
                }
                return _network_fee;
            }
        }
        ...
        public virtual bool Verify(Snapshot snapshot, IEnumerable<Transaction> mempool)
        {
            ...
            foreach (var group in Outputs.GroupBy(p => p.AssetId))
            {
                ...
                if (asset.Expiration <= snapshot.Height + 1 && asset.AssetType != AssetType.GoverningToken)
                    return false;
            ...
            switch (Type)
            {
                case TransactionType.BlockReward:
                    if (results_issue.Any(p => p.AssetId != Blockchain.GoverningToken.Hash))
                        return false;
                    break;
