...
namespace Krona.Consensus
{
    public sealed class ConsensusService : UntypedActor
    {
        ...
        private void OnPrepareRequestReceived(ConsensusPayload payload, PrepareRequest message)
        {
            ...
            foreach (Transaction tx in unverified)
                if (!AddTransaction(tx, true))
                    return;
            if (!AddTransaction(message.BlockReward, true)) return;
        ...
        {
            if (transaction.Type == TransactionType.BlockReward) return;
        ...
        private bool VerifyRequest()
        {
            if (!Blockchain.GetConsensusAddress(context.Snapshot.GetValidators(context.Transactions.Values).ToArray()).Equals(context.NextConsensus))
                return false;
            Transaction minerTx = context.Transactions.Values.FirstOrDefault(p => p.Type == TransactionType.BlockReward);

            Fixed8 amountNetFee = Block.CalculateNetFee(context.Transactions.Values);
            if (minerTx?.Outputs.Sum(p => p.Value) != amountNetFee + Blockchain.Singleton.GetBlockReward()) return false;
            return true;
        }
    }
