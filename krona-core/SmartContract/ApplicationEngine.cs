...
namespace Krona.SmartContract
{
    public class ApplicationEngine : ExecutionEngine
    {
        ...
        public new KronaService Service => (KronaService)base.Service;

        public ApplicationEngine(TriggerType trigger, IScriptContainer container, Snapshot snapshot, Fixed8 gas, bool testMode = false)
            : base(container, Cryptography.Crypto.Default, snapshot, new KronaService(trigger, snapshot))
        {
        ...
        protected virtual long GetPriceForSysCall()
        {
            ...
            if (api_hash == "Krona.Asset.Create".ToInteropMethodHash())
                return 5000L * 100000000L / ratio;
            if (api_hash == "Krona.Asset.Renew".ToInteropMethodHash())
                return (byte)CurrentContext.EvaluationStack.Peek(1).GetBigInteger() * 5000L * 100000000L / ratio;
            if (api_hash == "Krona.Contract.Create".ToInteropMethodHash() ||
                api_hash == "Krona.Contract.Migrate".ToInteropMethodHash())
            {
                long fee = 100L;
            ...
            if (api_hash == "System.Storage.Put".ToInteropMethodHash() ||
                ...
                api_hash == "Krona.Storage.Put".ToInteropMethodHash())
