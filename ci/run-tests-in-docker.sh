#!/bin/bash
#
# This script is run inside the Docker container and tests krona-cli
#
set -e
...
expect /opt/ci/test-krona-cli.expect

# Start krona-cli in background for additional JSON-RPC tests
screen -dmS node1 bash -c "dotnet krona-cli.dll --rpc"
