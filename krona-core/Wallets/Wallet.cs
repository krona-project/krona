...
namespace Krona.Wallets
{
    public abstract class Wallet : IDisposable
    {
        ...
        public T MakeTransaction<T>(T tx, UInt160 from = null, UInt160 change_address = null, Fixed8 fee = default(Fixed8)) where T : Transaction
        {
            ...
            {
                AssetId = p.AssetId,
                Unspents = from == null ? FindUnspentCoins(p.AssetId, p.Value + fee) : FindUnspentCoins(p.AssetId, p.Value + fee, from),
                IsFee = p.IsFee
            }).ToList();

            if (tx is not StateTransaction)
                pay_coins = pay_coins.Where(p => !p.IsFee).ToList();

            if (pay_coins.Any(p => p.Unspents == null)) return null;
            var input_sum = pay_coins.Select(p => new
            {
                AssetId = p.AssetId,
            ...
            Fixed8 in_sum = input_sum.Sum(q => q.Value);
            Fixed8 out_sum = pay_total.Sum(q => q.Value);
            ...
            if (in_sum > out_sum)
            {
                ...
                {
                    AssetId = Blockchain.GoverningToken.Hash,
                    Value = in_sum - out_sum,
                    ScriptHash = change_address
                });
            }

            tx.Inputs = pay_coins.SelectMany(p => p.Unspents).Select(p => p.Reference).ToArray();
